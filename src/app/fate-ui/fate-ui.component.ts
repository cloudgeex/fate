...
export const defaultButtons: Array<string | ButtonsGroup> = [
  {
    icon: 'heading',
    name: 'headers',
    buttons: [
  ...
  },
  {
    icon: 'list-ol',
    name: 'list',
    buttons: ['ordered', 'unordered']
  },
  {
    icon: 'align-left',
    name: 'alignment',
    buttons: ['center', 'justify', 'left', 'right']
  },
  'undo',
  'redo',
  'clean'
...
export class FateUiComponent implements OnChanges, AfterViewInit {
  ...
  constructor(
    ...
    public controller: FateControllerService,
    public icon: FateIconService,
  ...
  public do(event, action) {
    ...
    this.initDropdown(
      this.controller.getAction(action).dropdown,
      this.dropdownValue
    );
  ...
  public isButtonGroup(button: string | ButtonsGroup): boolean {
  ...
  public isButtonExcluded(button: string): boolean {
    if (this.excludeButtons.includes(button)) {
      return true;
    }
    return false;
  }

  public isAnyButtonEnabled(buttons: string[]): boolean {
    return buttons.some(button => {
      return Boolean(this.enabled[button]);
    });
  }
  ...
  public ngOnChanges(changes) {
    if (changes['uiId']) {
      ...
      this.inputSubscription = this.controller
        .enabled(this.uiId)
